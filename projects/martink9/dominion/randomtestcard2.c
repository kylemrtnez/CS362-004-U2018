...
* Date: 08/05/2018
* Description: This file contains a random test for the village card
...
int checkCard(int players, struct gameState *post, int ch1, int ch2, int ch3, int hp, int *bonus)
{
  ...
  cardEffect(village, ch1, ch2, ch3, post, hp, bonus);
...
/**********************************************************************
* testCard()
* Description: Tests the village card
* Parameters: 
...
int testCard(struct gameState *prestate, struct gameState *postState)
{
  ...
  // check handcounts - s/b equal (+1 card from village, discard village)
  int preHc = prestate->handCount[curPlayer];
  ...
  // check actions added  - s/b +2 from village
  int preActions = prestate->numActions + 2;
  int postActions = postState->numActions;

  if (preActions == postActions)
    printf("PASSED - pre-actions == post-actions\n");
  else
  {
    printf("FAILED - pre-actions != post-actions\n");
    testStatus = 0;
  }
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < 1000; i++)
  {
    ...
    // add village to be played
    handPos = GS.handCount[p];
    GS.handCount[p]++;
    GS.hand[p][handPos] = village;
